...
void *handle_events(void *a) {
    ...
    LOG_WARN("Start events tun=%d thread %x", args->tun, thread_id);
    ...
    if (getrlimit(RLIMIT_NOFILE, &rlim))
        ...
        LOG_WARN("getrlimit soft %d hard %d max sessions %d",
    ...
    if (epoll_fd < 0) {
        LOG_ERROR("epoll create error %d: %s", errno, strerror(errno));
    ...
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, pipefds[0], &ev_pipe)) {
        LOG_ERROR("epoll add pipe error %d: %s", errno, strerror(errno));
    ...
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, args->tun, &ev_tun)) {
        LOG_ERROR("epoll add tun error %d: %s", errno, strerror(errno));
    ...
    while (!stopping) {
        LOG_DEBUG("Loop thread %x", thread_id);
        ...
        if (ms - last_check > EPOLL_MIN_CHECK) {
            ...
            recheck = 1;
            LOG_DEBUG("Skipped session checks");
        }

        LOG_DEBUG("sessions ICMP %d UDP %d TCP %d max %d/%d timeout %d recheck %d",
        ...
        if (ready < 0) {
            if (errno == EINTR) {
                LOG_DEBUG("epoll interrupted tun %d thread %x", args->tun, thread_id);
                continue;
            }
            else {
                LOG_ERROR("epoll tun %d thread %x error %d: %s",
        ...
        if (ready == 0)
            ...
            for (int i = 0; i < ready; i++) {
                if (ev[i].data.ptr == &ev_pipe) {
                    ...
                    // Check upstream
                    LOG_DEBUG("epoll ready %d/%d in %d out %d err %d hup %d",
                                i, ready,
                    ...
                    // Check downstream
                    LOG_DEBUG("epoll ready %d/%d in %d out %d err %d hup %d prot %d sock %d",
                                i, ready,
                                (ev[i].events & EPOLLIN) != 0,
                                (ev[i].events & EPOLLOUT) != 0,
                                (ev[i].events & EPOLLERR) != 0,
    ...
    if (epoll_fd >= 0 && close(epoll_fd))
        LOG_ERROR("epoll close error %d: %s", errno, strerror(errno));
    ...
    LOG_WARN("Stopped events tun=%d thread %x", args->tun, thread_id);
    thread_id = 0;
    return NULL;
}
        ...
        if (s->protocol == IPPROTO_ICMP || s->protocol == IPPROTO_ICMPV6) {
            if (!s->icmp.stop) {
                ...
                if (is_address_allowed(args, objPacket) == NULL) {
                    s->icmp.stop = 1;
                    LOG_WARN("ICMP terminate %d uid %d",
                                s->socket, s->icmp.uid);
                }
            }
            ...
            if (s->udp.state == UDP_ACTIVE) {
                ...
                if (is_address_allowed(args, objPacket) == NULL) {
                    ...
                    LOG_WARN("UDP terminate session socket %d uid %d",
                                s->socket, s->udp.uid);
                }
            }
            else if (s->udp.state == UDP_BLOCKED) {
                LOG_WARN("UDP remove blocked session uid %d", s->udp.uid);
            ...
            if (s->tcp.state != TCP_CLOSING && s->tcp.state != TCP_CLOSE) {
                ...
                if (is_address_allowed(args, objPacket) == NULL) {
                    ...
                    LOG_WARN("TCP terminate socket %d uid %d",
                                s->socket, s->tcp.uid);
                }
            }
