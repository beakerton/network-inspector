    ...
    *qname = 0;
    ...
    while (len) {
        if (len & 0xC0) {
            ...
            len = *(data + ptr);
            LOG_DEBUG("DNS qname compression ptr %d len %d", ptr, len);
            if (!c) {
                c = 1;
                off += 2;
            }
    ...
    if (len > 0 || noff == 0) {
        LOG_ERROR("DNS qname invalid len %d noff %d", len, noff);
        return -1;
    }

    *(qname + noff - 1) = 0;
    LOG_DEBUG("qname %s", qname);
...
void parse_dns_response(const struct arguments *args, const struct udp_session *u,
    ...
    if (*datalen < sizeof(struct dns_header) + 1) {
        LOG_WARN("DNS response length %d", *datalen);
        return;
    }
    ...
    if (dns->qr == 1 && dns->opcode == 0 && qcount > 0 && acount > 0) {
        LOG_DEBUG("DNS response qcount %d acount %d", qcount, acount);
        if (qcount > 1)
            LOG_WARN("DNS response qcount %d acount %d", qcount, acount);
        ...
        for (int q = 0; q < 1; q++) {
            ...
            if (off > 0 && off + 4 <= *datalen) {
                ...
                if (q == 0) {
                    ...
                    LOG_DEBUG("DNS question %d qtype %d qclass %d qname %s",
                                q, qtype, qclass, qname);
                }
                off += 4;
            }
            else {
                LOG_WARN("DNS response Q invalid off %d datalen %d", off, *datalen);
                return;
            }
        }
            ...
            if (off > 0 && off + 10 <= *datalen) {
                ...
                if (off + rdlength <= *datalen) {
                    if (qclass == DNS_QCLASS_IN &&
                        ...
                        LOG_DEBUG("DNS answer %d qname %s qtype %d ttl %d data %s",
                                  a, name, qtype, ttl, rd);

                    }
                    else
                        LOG_DEBUG("DNS answer %d qname %s qclass %d qtype %d ttl %d length %d",
                                    a, name, qclass, qtype, ttl, rdlength);

                    off += rdlength;
                }
                else {
                    LOG_WARN("DNS response A invalid off %d rdlength %d datalen %d",
                                off, rdlength, *datalen);
                    return;
                }
            }
            else {
                LOG_WARN("DNS response A invalid off %d datalen %d", off, *datalen);
                return;
            }
        }
        ...
        LOG_WARN("DNS response qr %d opcode %d qcount %d acount %d",
...
int get_dns_query(const struct arguments *args, const struct udp_session *u,
    ...
    if (datalen < sizeof(struct dns_header) + 1) {
        LOG_WARN("DNS query length %d", datalen);
        return -1;
    }
    ...
    if (dns->qr == 0 && dns->opcode == 0 && qcount > 0) {
        if (qcount > 1)
            LOG_WARN("DNS query qcount %d", qcount);
        ...
        if (off > 0 && off + 4 == datalen) {
            ...
            *qclass = ntohs(*((uint16_t *) (data + off + 2)));
            ...
            LOG_WARN("DNS query invalid off %d datalen %d", off, datalen);
    }
...
int check_domain(const struct arguments *args, const struct udp_session *u,
    ...
    if (qclass == DNS_QCLASS_IN &&
        ...
        LOG_INFO("DNS query type %d name %s blocked", qtype, name);
        ...
        if (write_udp(args, u, response, rlen) < 0)
            LOG_WARN("UDP DNS write error %d: %s", errno, strerror(errno));
