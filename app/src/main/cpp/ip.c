...
int check_tun(const struct arguments *args,
    ...
    if (ev->events & EPOLLERR) {
        LOG_ERROR("tun %d exception", args->tun);
        if (fcntl(args->tun, F_GETFL) < 0) {
            LOG_ERROR("fcntl tun %d F_GETFL error %d: %s",
    ...
    if (ev->events & EPOLLIN) {
        ...
        if (length < 0) {
            ...
            LOG_ERROR("tun %d read error %d: %s",
                        args->tun, errno, strerror(errno));
            ...
            if (length > max_tun_msg) {
                max_tun_msg = length;
                LOG_WARN("Maximum tun msg length %d", max_tun_msg);
            }
            ...
            LOG_ERROR("tun %d empty read", args->tun);
...
void handle_ip(const struct arguments *args,
    ...
    if (version == 4) {
        if (length < sizeof(struct iphdr)) {
            LOG_WARN("IP4 packet too short length %d", length);
            return;
        }
        ...
        if (ip4hdr->frag_off & IP_MF) {
            LOG_ERROR("IP fragment offset %u",
        ...
        if (ntohs(ip4hdr->tot_len) != length) {
            LOG_ERROR("Invalid length %u header length %u",
        ...
        if (length < sizeof(struct ip6_hdr)) {
            LOG_WARN("IP6 packet too short length %d", length);
            return;
        }
        ...
        if (!is_upper_layer(protocol)) {
            LOG_WARN("IP6 extension %d", protocol);
            ...
            if (!is_upper_layer(protocol)) {
                off = 0;
                protocol = ip6hdr->ip6_nxt;
                LOG_WARN("IP6 final extension %d", protocol);
            }
        }
    ...
    if (protocol == IPPROTO_ICMP || protocol == IPPROTO_ICMPV6) {
        if (length - (payload - pkt) < sizeof(struct icmp)) {
            LOG_WARN("ICMP packet too short");
            return;
        }
        ...
        LOG_WARN("Unknown protocol %d", protocol);
    ...
    if (sessions >= maxsessions) {
        if ((protocol == IPPROTO_ICMP || protocol == IPPROTO_ICMPV6) ||
            ...
            LOG_ERROR("%d of max %d sessions, dropping version %d protocol %d",
    ...
    LOG_DEBUG("Packet v%d %s/%u > %s/%u proto %d flags %s uid %d",
    ...
    if (allowed) {
        ...
        LOG_WARN("Address v%d p%d %s/%u syn %d not allowed",
...
jint get_uid(const int version, const int protocol,
    ...
    char dest[INET6_ADDRSTRLEN + 1];
    inet_ntop(version == 4 ? AF_INET : AF_INET6, daddr, dest, sizeof(dest));
    LOG_INFO("get uid v%d p%d %u > %s/%u",
    ...
    if (uid < 0)
        LOG_ERROR("uid v%d p%d %u > %s/%u not found",
                    version, protocol, sport, dest, dport);
...
jint get_uid_sub(const int version, const int protocol,
    ...
    FILE *fd = fopen(fn, "r");
    if (fd == NULL) {
        LOG_ERROR("fopen %s error %d: %s", fn, errno, strerror(errno));
        return uid;
    }
    ...
    *line = 0;
        ...
        if (i++) {
            *hex = 0;
            ...
            if (fields == 4 && (version == 4 ? strlen(hex) == 8 : strlen(hex) == 32)) {
                ...
                LOG_ERROR("Invalid field #%d: %s", fields, line);
        }
    }

    if (fclose(fd))
        LOG_ERROR("fclose %s error %d: %s", fn, errno, strerror(errno));
    ...
    if (mselapsed > PROFILE_UID)
        LOG_WARN("get uid ip %f", mselapsed);
#endif
