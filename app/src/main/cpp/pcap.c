...
void write_pcap_rec(const uint8_t *buffer, size_t length) {
    ...
    if (clock_gettime(CLOCK_REALTIME, &ts))
        LOG_ERROR("clock_gettime error %d: %s", errno, strerror(errno));
...
void write_pcap(const void *ptr, size_t len) {
    if (fwrite(ptr, len, 1, pcap_file) < 1)
        LOG_ERROR("PCAP fwrite error %d: %s", errno, strerror(errno));
    else {
        long fsize = ftell(pcap_file);
        LOG_INFO(ANDROID_LOG_VERBOSE, "PCAP wrote %d @%ld", len, fsize);

        if (fsize > pcap_file_size) {
            LOG_WARN("PCAP truncate @%ld", fsize);
            if (ftruncate(fileno(pcap_file), sizeof(struct pcap_hdr_s)))
                LOG_ERROR("PCAP ftruncate error %d: %s",
