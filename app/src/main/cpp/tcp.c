/*
    ...
    Modifications Copyright 2017 Adam Herchenroether
    Full modifications can be viewed in git history at https://github.com/beakerton/network-inspector
*/
...
int check_tcp_session(const struct arguments *args, struct ng_session *s,
    ...
    if (s->tcp.state != TCP_CLOSING && s->tcp.state != TCP_CLOSE &&
        ...
        LOG_WARN("%s idle %d/%d sec ", session, now - s->tcp.time,
                    timeout);
    ...
    if (s->tcp.state == TCP_CLOSING) {
        ...
        if (s->socket >= 0) {
            if (close(s->socket))
                LOG_ERROR("%s close error %d: %s",
                            session, errno, strerror(errno));
            else
                LOG_WARN("%s close", session);
            s->socket = -1;
        }
...
int monitor_tcp_session(const struct arguments *args, struct ng_session *s, int epoll_fd) {
    ...
    if (s->tcp.state == TCP_LISTEN) {
        ...
        if (get_send_window(&s->tcp) > 0)
            ...
            if (ms - s->tcp.last_keep_alive > EPOLL_MIN_CHECK) {
                ...
                LOG_WARN("Sending keep alive to update send window");
                s->tcp.remote_seq--;
    ...
    if (events != s->ev.events) {
        ...
        if (epoll_ctl(epoll_fd, EPOLL_CTL_MOD, s->socket, &s->ev)) {
            s->tcp.state = TCP_CLOSING;
            LOG_ERROR("epoll mod tcp error %d: %s", errno, strerror(errno));
        }
        else
            LOG_DEBUG("epoll mod tcp socket %d in %d out %d",
...
int get_receive_buffer(const struct ng_session *cur) {
    ...
    if (getsockopt(cur->socket, SOL_SOCKET, SO_SNDBUF, &sendbuf, &sendbufsize) < 0)
        LOG_WARN("getsockopt SO_RCVBUF %d: %s", errno, strerror(errno));
    ...
    if (ioctl(cur->socket, SIOCOUTQ, &unsent))
        LOG_WARN("ioctl SIOCOUTQ %d: %s", errno, strerror(errno));
...
void check_tcp_socket(const struct arguments *args,
    ...
    if (ev->events & EPOLLERR) {
        ...
        if (err < 0)
            LOG_ERROR("%s getsockopt error %d: %s",
                        session, errno, strerror(errno));
        else if (serr)
            LOG_ERROR("%s SO_ERROR %d: %s",
        ...
        if (s->tcp.state == TCP_LISTEN) {
            ...
            if (s->tcp.socks5 == SOCKS5_NONE) {
                ...
                if (ev->events & EPOLLIN) {
                    ...
                    if (bytes < 0) {
                        LOG_ERROR("%s recv SOCKS5 error %d: %s",
                                    session, errno, strerror(errno));
                        write_rst(args, &s->tcp);
                    }
                    else {
                        char *h = hex(buffer, (const size_t) bytes);
                        LOG_INFO("%s recv SOCKS5 %s", session, h);
                        free(h);
                        ...
                        if (s->tcp.socks5 == SOCKS5_HELLO &&
                            ...
                            if (buffer[1] == 0)
                                ...
                                s->tcp.socks5 = 0;
                                LOG_ERROR("%s SOCKS5 auth %d not supported",
                                            session, buffer[1]);
                                write_rst(args, &s->tcp);
                            }
                            ...
                            if (buffer[1] == 0) {
                                s->tcp.socks5 = SOCKS5_CONNECT;
                                LOG_WARN("%s SOCKS5 auth OK", session);
                            }
                            else {
                                s->tcp.socks5 = 0;
                                LOG_ERROR("%s SOCKS5 auth error %d",
                                            session, buffer[1]);
                                write_rst(args, &s->tcp);
                            }
                            ...
                            if (buffer[1] == 0) {
                                s->tcp.socks5 = SOCKS5_CONNECTED;
                                LOG_WARN("%s SOCKS5 connected", session);
                            }
                            else {
                                s->tcp.socks5 = 0;
                                LOG_ERROR("%s SOCKS5 connect error %d",
                                            session, buffer[1]);
                                write_rst(args, &s->tcp);
                                /*
                            ...
                            s->tcp.socks5 = 0;
                            LOG_ERROR("%s recv SOCKS5 state %d",
            ...
            if (s->tcp.socks5 == SOCKS5_HELLO) {
                ...
                LOG_INFO("%s sending SOCKS5 hello: %s",
                            session, h);
                free(h);
                ssize_t sent = send(s->socket, buffer, sizeof(buffer), MSG_NOSIGNAL);
                if (sent < 0) {
                    LOG_ERROR("%s send SOCKS5 hello error %d: %s",
                                session, errno, strerror(errno));
                    write_rst(args, &s->tcp);
                }
                ...
                *(buffer + 2 + ulen) = plen;
                ...
                LOG_INFO("%s sending SOCKS5 auth: %s",
                            session, h);
                free(h);
                ssize_t sent = send(s->socket, buffer, len, MSG_NOSIGNAL);
                if (sent < 0) {
                    LOG_ERROR("%s send SOCKS5 connect error %d: %s",
                                session, errno, strerror(errno));
                    write_rst(args, &s->tcp);
                }
                ...
                *(buffer + 3) = (uint8_t) (s->tcp.version == 4 ? 1 : 4);
                ...
                char *h = hex(buffer, len);
                LOG_INFO("%s sending SOCKS5 connect: %s",
                            session, h);
                free(h);
                ssize_t sent = send(s->socket, buffer, len, MSG_NOSIGNAL);
                if (sent < 0) {
            ...
            if (s->tcp.state == TCP_ESTABLISHED || s->tcp.state == TCP_CLOSE_WAIT) {
                ...
                if ((ev->events & EPOLLIN) && send_window > 0) {
                    ...
                    if (bytes < 0) {
                        ...
                        LOG_WARN("%s recv eof", session);

                        if (s->tcp.forward == NULL) {
                            if (write_fin_ack(args, &s->tcp) >= 0) {
                                LOG_WARN("%s FIN sent", session);
                            ...
                            if (s->tcp.state == TCP_ESTABLISHED)
                                ...
                                LOG_ERROR("%s invalid close", session);
                        }
                        else {
                            // There was still data to send
                            LOG_ERROR("%s close with queue", session);
                            write_rst(args, &s->tcp);
                        }
                        ...
                        // Socket read data
                        LOG_DEBUG("%s recv bytes %d", session, bytes);
...
jboolean handle_tcp(const struct arguments *args,
    ...
    //tcphdr->urg ? LOG_WARN(packet) : LOG_DEBUG(packet);
    ...
    if (cur == NULL) {
        if (tcphdr->syn) {
            ...
            LOG_WARN("%s new session mss %u ws %u window %u",
            ...
            LOG_DEBUG("TCP socket %d lport %d",
            ...
            if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, s->socket, &s->ev))
                LOG_ERROR("epoll add tcp error %d: %s",
            ...
            if (!allowed) {
                LOG_WARN("%s resetting blocked session", packet);
                write_rst(args, &s->tcp);
            }
        }
        else {
            LOG_WARN("%s unknown session", packet);
        ...
        if (cur->tcp.state == TCP_CLOSING || cur->tcp.state == TCP_CLOSE) {
            ...
            if (datalen) {
                if (cur->socket < 0) {
                    LOG_ERROR("%s data while local closed", session);
                    write_rst(args, &cur->tcp);
                    return 0;
                }
                if (cur->tcp.state == TCP_CLOSE_WAIT) {
                    LOG_ERROR("%s data while remote closed", session);
                    write_rst(args, &cur->tcp);
                    return 0;
                }
            ...
            if (tcphdr->rst /* +ACK */) {
                ...
                LOG_WARN("%s received reset", session);
                ...
                if (!tcphdr->ack || ntohl(tcphdr->ack_seq) == cur->tcp.local_seq) {
                    if (tcphdr->syn) {
                        ...
                        if (cur->tcp.state == TCP_ESTABLISHED) {
                            ...
                            LOG_ERROR("%s invalid FIN", session);
                            return 0;
                        }
                        ...
                        if (cur->tcp.state == TCP_SYN_RECV)
                            ...
                            LOG_ERROR("%s invalid state", session);
                            return 0;
                        }
                    }

                    else {
                        LOG_ERROR("%s unknown packet", session);
                        return 0;
                    }
                }
                else {
                    ...
                    if ((uint32_t) (ack + 1) == cur->tcp.local_seq) {
                        ...
                        if (cur->tcp.state == TCP_ESTABLISHED) {
                            ...
                            if (setsockopt(cur->socket, SOL_SOCKET, SO_KEEPALIVE, &on, sizeof(on)))
                                LOG_ERROR("%s setsockopt SO_KEEPALIVE error %d: %s",
                                            session, errno, strerror(errno));
                            else
                                LOG_WARN("%s enabled keep alive", session);
                        }
                        else
                            LOG_WARN("%s keep alive", session);

                    }
                    else if (compare_u32(ack, cur->tcp.local_seq) < 0) {
                        if (compare_u32(ack, cur->tcp.acked) <= 0)
                            LOG_WARN("%s repeated ACK %u/%u",
                                    session,
                                    ack - cur->tcp.local_start,
                                    cur->tcp.acked - cur->tcp.local_start);
                        else {
                            LOG_WARN("%s previous ACK %u",
                        ...
                        LOG_ERROR("%s future ACK", session);
                        write_rst(args, &cur->tcp);
                        return 0;
                    }
                }
...
void queue_tcp(const struct arguments *args,
    ...
    if (compare_u32(seq, cur->remote_seq) < 0)
        LOG_WARN("%s already forwarded %u..%u",
                    session,
        ...
        if (s == NULL || compare_u32(s->seq, seq) > 0) {
            ...
            if (s->len == datalen)
                ...
                LOG_ERROR("%s segment larger %u..%u < %u",
                            session,
                            s->seq - cur->remote_start, s->seq + s->len - cur->remote_start,
                            s->seq + datalen - cur->remote_start);
        }
...
int open_tcp_socket(const struct arguments *args,
    ...
    if (flags < 0 || fcntl(sock, F_SETFL, flags | O_NONBLOCK) < 0) {
        LOG_ERROR("fcntl socket O_NONBLOCK error %d: %s",
                    errno, strerror(errno));
        return -1;
    }
    ...
    if (redirect == NULL) {
        ...
        LOG_WARN("TCP%d redirect to %s/%u",
...
ssize_t write_tcp(const struct arguments *args, const struct tcp_session *cur,
    ...
    // Send packet
    LOG_DEBUG("TCP sending%s%s%s%s to tun %s/%u seq %u ack %u data %u",
                (tcp->syn ? " SYN" : ""),
                (tcp->ack ? " ACK" : ""),
                (tcp->fin ? " FIN" : ""),
                (tcp->rst ? " RST" : ""),
    ...
    if (res >= 0) {
        ...
        LOG_ERROR("TCP write%s%s%s%s data %d error %d: %s",
                    (tcp->syn ? " SYN" : ""),
                    (tcp->ack ? " ACK" : ""),
                    (tcp->fin ? " FIN" : ""),
                    (tcp->rst ? " RST" : ""),
    ...
    if (res != len) {
        LOG_ERROR("TCP write %d/%d", res, len);
        return -1;
    }
