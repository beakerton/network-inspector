...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    if ((*vm)->GetEnv(vm, (void **) &env, JNI_VERSION_1_6) != JNI_OK) {
        LOG_INFO("JNI load GetEnv failed");
        return -1;
    }
    ...
    if (getrlimit(RLIMIT_NOFILE, &rlim))
        LOG_WARN("getrlimit error %d: %s", errno, strerror(errno));
    else {
        rlim_t soft = rlim.rlim_cur;
        rlim.rlim_cur = rlim.rlim_max;
        if (setrlimit(RLIMIT_NOFILE, &rlim))
            LOG_WARN("setrlimit error %d: %s", errno, strerror(errno));
        else
            LOG_WARN("raised file limit from %d to %d", soft, rlim.rlim_cur);
    }
...
Java_net_herchenroether_networkinspector_vpn_PacketCaptureService_jni_1init(JNIEnv *env, jobject instance) {
    ...
    *socks5_password = 0;
    ...
    // Create signal pipe
    if (pipe(pipefds))
        LOG_ERROR("Create pipe error %d: %s", errno, strerror(errno));
    else
        for (int i = 0; i < 2; i++) {
            int flags = fcntl(pipefds[i], F_GETFL, 0);
            if (flags < 0 || fcntl(pipefds[i], F_SETFL, flags | O_NONBLOCK) < 0)
                LOG_ERROR("fcntl pipefds[%d] O_NONBLOCK error %d: %s",
                            i, errno, strerror(errno));
        }
}

JNIEXPORT void JNICALL
Java_net_herchenroether_networkinspector_vpn_PacketCaptureService_jni_1start(
        JNIEnv *env, jobject instance, jint tun, jboolean fwd53, jint rcode, jint loglevel_) {

    loglevel = loglevel_;
    max_tun_msg = 0;
    LOG_WARN("Starting tun %d fwd53 %d level %d thread %x",
                tun, fwd53, loglevel, thread_id);

    // Set blocking
    int flags = fcntl(tun, F_GETFL, 0);
    if (flags < 0 || fcntl(tun, F_SETFL, flags & ~O_NONBLOCK) < 0)
        LOG_ERROR("fcntl tun ~O_NONBLOCK error %d: %s",
                    errno, strerror(errno));

    if (thread_id && pthread_kill(thread_id, 0) == 0)
        LOG_ERROR("Already running thread %x", thread_id);
    else {
        jint rs = (*env)->GetJavaVM(env, &jvm);
        if (rs != JNI_OK)
            LOG_ERROR("GetJavaVM failed");
        ...
        if (err == 0)
            LOG_WARN("Started thread %x", thread_id);
        else
            LOG_ERROR("pthread_create error %d: %s", err, strerror(err));
    }
}

JNIEXPORT void JNICALL
Java_net_herchenroether_networkinspector_vpn_PacketCaptureService_jni_1stop(
        JNIEnv *env, jobject instance, jint tun, jboolean clr) {
    pthread_t t = thread_id;
    LOG_WARN("Stop tun %d  thread %x", tun, t);
    if (t && pthread_kill(t, 0) == 0) {
        LOG_WARN("Write pipe thread %x", t);
        if (write(pipefds[1], "x", 1) < 0)
            LOG_WARN("Write pipe error %d: %s", errno, strerror(errno));
        else {
            LOG_WARN("Join thread %x", t);
            int err = pthread_join(t, NULL);
            if (err != 0)
                LOG_WARN("pthread_join error %d: %s", err, strerror(err));
        }

        if (clr)
            clear();

        LOG_WARN("Stopped thread %x", t);
    }
    else
        LOG_WARN("Not running thread %x", t);
}

JNIEXPORT jint JNICALL
Java_net_herchenroether_networkinspector_vpn_PacketCaptureService_jni_1get_1mtu(JNIEnv *env, jobject instance) {
    return get_mtu();
}

JNIEXPORT jintArray JNICALL
Java_net_herchenroether_networkinspector_vpn_PacketCaptureService_jni_1get_1stats(JNIEnv *env, jobject instance) {
...
JNIEXPORT void JNICALL
Java_net_herchenroether_networkinspector_vpn_PacketCaptureService_jni_1pcap(
    ...
    if (name_ == NULL) {
        if (pcap_file != NULL) {
            ...
            if (flags < 0 || fcntl(fileno(pcap_file), F_SETFL, flags & ~O_NONBLOCK) < 0)
                LOG_ERROR("PCAP fcntl ~O_NONBLOCK error %d: %s",
                            errno, strerror(errno));

            if (fsync(fileno(pcap_file)))
                LOG_ERROR("PCAP fsync error %d: %s", errno, strerror(errno));

            if (fclose(pcap_file))
                LOG_ERROR("PCAP fclose error %d: %s", errno, strerror(errno));

            pcap_file = NULL;
        }
        LOG_WARN("PCAP disabled");
    }
    else {
        const char *name = (*env)->GetStringUTFChars(env, name_, 0);
        LOG_WARN("PCAP file %s record size %d truncate @%ld",
                    name, pcap_record_size, pcap_file_size);

        pcap_file = fopen(name, "ab+");
        if (pcap_file == NULL)
            LOG_ERROR("PCAP fopen error %d: %s", errno, strerror(errno));
        else {
            int flags = fcntl(fileno(pcap_file), F_GETFL, 0);
            if (flags < 0 || fcntl(fileno(pcap_file), F_SETFL, flags | O_NONBLOCK) < 0)
                LOG_ERROR("PCAP fcntl O_NONBLOCK error %d: %s",
            ...
            if (size == 0) {
                LOG_WARN("PCAP initialize");
                write_pcap_hdr();
            }
            else
                LOG_WARN("PCAP current size %ld", size);
        }
...
JNIEXPORT void JNICALL
Java_net_herchenroether_networkinspector_vpn_PacketCaptureService_jni_1socks5(JNIEnv *env, jobject instance, jstring addr_,
...
JNIEXPORT void JNICALL
Java_net_herchenroether_networkinspector_vpn_PacketCaptureService_jni_1done(JNIEnv *env, jobject instance) {
    LOG_INFO("Done");
    ...
    for (int i = 0; i < 2; i++)
        if (close(pipefds[i]))
            LOG_ERROR("Close pipe error %d: %s", errno, strerror(errno));
}
...
int protect_socket(const struct arguments *args, int socket) {
    ...
    if (!isProtected) {
        LOG_ERROR("protect socket failed");
        return -1;
    }
...
jobject jniGlobalRef(JNIEnv *env, jobject cls) {
    ...
    if (gcls == NULL)
        LOG_ERROR("Global ref failed (out of memory?)");
    return gcls;
}
...
jclass jniFindClass(JNIEnv *env, const char *name) {
    ...
    if (cls == NULL)
        LOG_ERROR("Class %s not found", name);
    else
...
jmethodID jniGetMethodID(JNIEnv *env, jclass cls, const char *name, const char *signature) {
    ...
    if (method == NULL) {
        LOG_ERROR("Method %s %s not found", name, signature);
        jniCheckException(env);
    }
    return method;
}
...
jfieldID jniGetFieldID(JNIEnv *env, jclass cls, const char *name, const char *type) {
    ...
    if (field == NULL)
        LOG_ERROR("Field %s type %s not found", name, type);
    return field;
}
...
jobject jniNewObject(JNIEnv *env, jclass cls, jmethodID constructor, const char *name) {
    ...
    if (object == NULL)
        LOG_ERROR("Create object %s failed", name);
    else
        jniCheckException(env);
    return object;
}
...
void log_packet(const struct arguments *args, jobject jpacket) {
    ...
    if (mselapsed > PROFILE_JNI)
        LOG_WARN("log_packet %f", mselapsed);
#endif
}
...
jboolean is_domain_blocked(const struct arguments *args, const char *name) {
    ...
    mselapsed = (end.tv_sec - start.tv_sec) * 1000.0 +
                (end.tv_usec - start.tv_usec) / 1000.0;
    if (mselapsed > PROFILE_JNI)
        LOG_WARN("is_domain_blocked %f", mselapsed);
#endif
...
struct allowed *is_address_allowed(const struct arguments *args, jobject jpacket) {
    ...
    mselapsed = (end.tv_sec - start.tv_sec) * 1000.0 +
                (end.tv_usec - start.tv_usec) / 1000.0;
    if (mselapsed > PROFILE_JNI)
        LOG_WARN("is_address_allowed %f", mselapsed);
#endif
...
jobject create_packet(const struct arguments *args,
    ...
    mselapsed = (end.tv_sec - start.tv_sec) * 1000.0 +
                (end.tv_usec - start.tv_usec) / 1000.0;
    if (mselapsed > PROFILE_JNI)
        LOG_WARN("create_packet %f", mselapsed);
#endif
