...
int check_icmp_session(const struct arguments *args, struct ng_session *s,
    ...
    if (s->icmp.stop || s->icmp.time + timeout < now) {
        ...
        LOG_WARN("ICMP idle %d/%d sec stop %d from %s to %s",
                    now - s->icmp.time, timeout, s->icmp.stop, dest, source);

        if (close(s->socket))
            LOG_ERROR("ICMP close %d error %d: %s",
...
void check_icmp_socket(const struct arguments *args, const struct epoll_event *ev) {
    ...
    if (ev->events & EPOLLERR) {
        ...
        if (err < 0)
            LOG_ERROR("ICMP getsockopt error %d: %s",
                        errno, strerror(errno));
        else if (serr)
            LOG_ERROR("ICMP SO_ERROR %d: %s",
                        serr, strerror(serr));
        ...
        if (ev->events & EPOLLIN) {
            ...
            if (bytes < 0) {
                ...
                LOG_INFO("ICMP recv bytes %d from %s for tun type %d code %d id %x/%x seq %d",
                        bytes, dest,
...
jboolean handle_icmp(const struct arguments *args,
    ...
    if (icmp->icmp_type != ICMP_ECHO) {
        LOG_WARN("ICMP type %d code %d from %s to %s not supported",
    ...
    if (cur == NULL) {
        LOG_INFO("ICMP new session from %s to %s", source, dest);
        ...
        LOG_DEBUG("ICMP socket %d id %x", s->socket, s->icmp.id);
        ...
        if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, s->socket, &s->ev))
            LOG_ERROR("epoll add icmp error %d: %s", errno, strerror(errno));
    ...
    LOG_INFO("ICMP forward from tun %s to %s type %d code %d id %x seq %d data %d",
             source, dest,
    ...
    if (sendto(cur->socket, icmp, (socklen_t) icmplen, MSG_NOSIGNAL,
        ...
        LOG_ERROR("ICMP sendto error %d: %s", errno, strerror(errno));
        if (errno != EINTR && errno != EAGAIN) {
            cur->icmp.stop = 1;
            return 0;
        }
...
int open_icmp_socket(const struct arguments *args, const struct icmp_session *cur) {
    ...
    if (sock < 0) {
        LOG_ERROR("ICMP socket error %d: %s", errno, strerror(errno));
        return -1;
    }
...
ssize_t write_icmp(const struct arguments *args, const struct icmp_session *cur,
    ...
    // Send raw ICMP message
    LOG_WARN("ICMP sending to tun %d from %s to %s data %u type %d code %d id %x seq %d",
    ...
    if (res >= 0) {
        ...
        LOG_WARN("ICMP write error %d: %s", errno, strerror(errno));
