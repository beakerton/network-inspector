...
int check_udp_session(const struct arguments *args, struct ng_session *s,
    ...
    if (s->udp.state == UDP_ACTIVE && s->udp.time + timeout < now) {
        LOG_WARN("UDP idle %d/%d sec state %d from %s/%u to %s/%u",
    ...
    if (s->udp.state == UDP_FINISHING) {
        LOG_INFO("UDP close from %s/%u to %s/%u socket %d",
                    source, ntohs(s->udp.source), dest, ntohs(s->udp.dest), s->socket);

        if (close(s->socket))
            LOG_ERROR("UDP close %d error %d: %s",
...
void check_udp_socket(const struct arguments *args, const struct epoll_event *ev) {
    ...
    if (ev->events & EPOLLERR) {
        ...
        if (err < 0)
            LOG_ERROR("UDP getsockopt error %d: %s",
                        errno, strerror(errno));
        else if (serr)
            LOG_ERROR("UDP SO_ERROR %d: %s", serr, strerror(serr));
        ...
        if (ev->events & EPOLLIN) {
            ...
            if (bytes < 0) {
                ...
                if (s->udp.version == 4)
                    ...
                    inet_ntop(AF_INET6, &s->udp.daddr.ip6, dest, sizeof(dest));
                LOG_INFO("UDP recv bytes %d from %s/%u for tun",
...
void block_udp(const struct arguments *args,
    ...
    LOG_INFO("UDP blocked session from %s/%u to %s/%u",
...
jboolean handle_udp(const struct arguments *args,
    ...
    if (cur != NULL && cur->udp.state != UDP_ACTIVE) {
        LOG_INFO("UDP ignore session from %s/%u to %s/%u state %d",
    ...
    if (cur == NULL) {
        LOG_INFO("UDP new session from %s/%u to %s/%u",
                    source, ntohs(udphdr->source), dest, ntohs(udphdr->dest));
        ...
        LOG_DEBUG("UDP socket %d", s->socket);
        ...
        if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, s->socket, &s->ev))
            LOG_ERROR("epoll add udp error %d: %s", errno, strerror(errno));
    ...
    if (ntohs(udphdr->dest) == 53) {
        ...
        if (get_dns_query(args, &cur->udp, data, datalen, &qtype, &qclass, qname) >= 0) {
            LOG_DEBUG("DNS query qtype %d qclass %d name %s",
                        qtype, qclass, qname);
    ...
    LOG_INFO("UDP forward from tun %s/%u to %s/%u data %d",
    ...
    if (redirect == NULL) {
        ...
        rversion = (strstr(redirect->raddr, ":") == NULL ? 4 : 6);
        LOG_WARN("UDP%d redirect to %s/%u",
    ...
    if (sendto(cur->socket, data, (socklen_t) datalen, MSG_NOSIGNAL,
        ...
        LOG_ERROR("UDP sendto error %d: %s", errno, strerror(errno));
        if (errno != EINTR && errno != EAGAIN) {
            cur->udp.state = UDP_FINISHING;
            return 0;
        }
...
int open_udp_socket(const struct arguments *args,
    ...
    if (sock < 0) {
        LOG_ERROR("UDP socket error %d: %s", errno, strerror(errno));
        return -1;
    }
    ...
    if (cur->version == 4) {
        ...
        if (memcmp(&cur->daddr.ip4, &broadcast4, sizeof(broadcast4)) == 0) {
            LOG_WARN("UDP4 broadcast");
            int on = 1;
            if (setsockopt(sock, SOL_SOCKET, SO_BROADCAST, &on, sizeof(on)))
                LOG_ERROR("UDP setsockopt SO_BROADCAST error %d: %s",
                            errno, strerror(errno));
        }
    }
    else {
        ...
        if (*((uint8_t *) &cur->daddr.ip6) == 0xFF) {
            ...
            if (setsockopt(sock, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &mreq6, sizeof(mreq6)))
                LOG_ERROR("UDP setsockopt IPV6_ADD_MEMBERSHIP error %d: %s",
                            errno, strerror(errno));
        }
    }
...
ssize_t write_udp(const struct arguments *args, const struct udp_session *cur,
    ...
    // Send packet
    LOG_DEBUG("UDP sending to tun %d from %s/%u to %s/%u data %u",
    ...
    if (res >= 0) {
        ...
        LOG_WARN("UDP write error %d: %s", errno, strerror(errno));

    free(buffer);

    if (res != len) {
        LOG_ERROR("write %d/%d", res, len);
        return -1;
    }
